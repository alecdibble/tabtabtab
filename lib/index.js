 ...
 *
 */
const install = async (options = { name: '', completer: '', shell: null, application_name: 'tabtabtab' }) => {
  ...
  return prompt(shell, application_name).then(({ location }) =>
    installer.install({
      name,
      completer,
      location
...
const installMultiple = async (options, shell: null, application_name: 'tabtabtab') => {
  if (!options.isArray()) throw new TypeError('options must be an array');
